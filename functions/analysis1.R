...
###center time
#move all to 0
dataset.rt$endTime2=dataset.rt$endTime
dataset.rt$endTime2[which(dataset.rt$block=="main3")]=dataset.rt$endTime[which(dataset.rt$block=="main3")]-20/30
dataset.rt$endTime2[which(dataset.rt$block=="main2")]=dataset.rt$endTime[which(dataset.rt$block=="main2")]-10/30
center=function(var,group) {
  return(var-tapply(var,group,mean,na.rm=T)[group])
}
dataset.rt$endTime=center(dataset.rt$endTime,dataset.rt$block)
...
mBaseTimeCovariate2=lmer(reactionTime~deg*block+endTime2+deg*correctSide+MRexperience+(deg+block|ID)+(1|modelNumber),data=dataset.rt,REML=FALSE,control = lmerControl(optimizer = "optimx",optCtrl = list(method = "bobyqa")))
